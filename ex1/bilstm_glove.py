...
from utils import (seed_everything, load_ag_news_subset,
                   compute_classification_metrics, get_device,
                   load_needed_glove_vectors, build_glove_embedding_matrix)
import numpy as np
import torch
...
def main():
    ...
    for seed in SEEDS:
        ...
        # Chargement GloVe depuis Stanford (comme dans les TP)
        glove_vectors, glove_dim = load_needed_glove_vectors(set(vocab.keys()))
        glove_matrix, coverage = build_glove_embedding_matrix(vocab, glove_vectors, glove_dim)
        print(f"  GloVe coverage: {coverage:.2%}")
        glove_emb = torch.FloatTensor(glove_matrix)
