""" 2000 exemples d'entraînement, 3 epochs, lr=3e-4
"""
import sys, os
sys.path.append(os.path.join(os.path.dirname(__file__), ".."))

from utils import (seed_everything, compute_rouge_n, compute_rouge_l,
                   compute_embedding_similarity, get_device)
import numpy as np
import torch
...
def main():
    ...
    for seed in SEEDS:
        ...
        for dialogue in test_dial:
            ...
            with torch.no_grad():
                output = model.generate(**inputs, max_length=MAX_OUTPUT_LEN, num_beams=BEAM_SIZE, do_sample=False, early_stopping=True)
        ...
        emb_sims = compute_embedding_similarity(candidates, test_sum, device)
